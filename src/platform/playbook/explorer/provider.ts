...
export class PlaybookProvider implements vscode.TreeDataProvider<OutlineNode> {
  ...
  getChildren(node?: OutlineNode): OutlineNode[] {
    if (node) {
      const children = node.getChildren(); //this.applySearch(node.getChildren());
      if (children.length > 2) {
        return children; //this.sortChildren(children);
      }
      return children;
    }
    if (this.rootNode) {
      return this.rootNode.getChildren(); //this.sortRootChildren(this.rootNode.getChildren());
    }
    return [];
  }
