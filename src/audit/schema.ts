...
function getSchemaForObject(json: any, schema?: ObjectSchema): ObjectSchema {
  ...
  for (const key in json) {
    ...
    if (type === 'object') {
      ...
      schema.properties[key] = isMixedTypeArray(value) ? getSchemaForMixedArray(value) : getSchemaForUniformArray(value);
    }
...
export function generateSchema(json: any): ObjectSchema | ArraySchema {
  ...
  if (result.type === 'object') {
    ...
    const schema = isMixedTypeArray(json) ? getSchemaForMixedArray(json) : getSchemaForUniformArray(json);
    result.type = schema.type;
