...
The extension integrates with [API Security Audit](https://42crunch.com/free-user-faq/) and the [API Security Scan](https://42crunch.com/free-user-faq/) by 42Crunch, so that you can check the quality and the security of your APIs directly in VS Code.

If you are a 42Crunch customer or have an account of the 42Crunch platform, you can also directly access API collections and security reports through the integrated platform navigator.
...
We also encourage you to watch [this video](https://vimeo.com/871716353/1d7e2ecbdc?share=copy) that gives you a full tour of the editor and its different features.
...
![](https://img.shields.io/badge/Learning%20More!-red) Watch this [video]() on editor basics. 
...
![](https://img.shields.io/badge/Learning%20More!-green) Watch this [video](https://42crunch.com/free-user-faq/?utm_campaign=IDE&utm_medium=referral&utm_source=vscode#Free-API-Security-Testing-FAQs-0) to learn more about 42Crunch  Security Audit.
...
![AuditIssuesList](/Volumes/MYDATA/Product/vscode-openapi/images/AuditIssuesList.png)

#### Full Issues list

The full issues list contains all issues found. The list can be filtered in two ways:

* **Using the security gates toggle**: security gates are enforcing enterprise compliance and will hightlight issues that are offending the requirements established, such as a minimal score, issues severity or specific issues (e.g. an API key is used when only OAuth is allowed across the enterprise). When the toggle is on, only the issues affecting the security gates status are shown.


![](https://img.shields.io/badge/Note%20-blue) Security Quality Gates results are not visible yet to all Freemium users.
...
For each issue, you have access to full information about the issue, why it's relevant and recommendations on how to address the issue. 

![](https://img.shields.io/badge/Learning%20More!-green) Watch those [videos](https://42crunch.com/tutorial-openapi-swagger-extension-vs-code/) to learn more about audit and how to navigate issues.
...
![](https://img.shields.io/badge/Important-red)  You should only use 42Crunch Scan against APIs that you own, but not those of third parties.
...
When you first launch a scan, you will be presented with the scan configuration viewer. The scan configuration is generated automatically from the OpenAPI file you chose to scan. 
...
In order to use the 42Crunch Freemium services, we need access to the following URL: https://stateless.42crunch.com. You may need to ask your administrators to add a firewall rule to allow the connection.
