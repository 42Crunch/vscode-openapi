...
Identities are primarily related to authorization aspects of your operations.
...
To effectively test such APIs, you can create distinct Identites for "ordinary" and
"admin" users, configuring respective operations to use one or the other.

### Secrets

You may need to use secrets like passwords or API keys in Identity.
...
and then set Identity to `{{password1}}`.
...
To achieve this, you can add a call to the "login" operation within Identity, extract the token from its
response and store it in a variable named `token`. Then, use the expression `{{token}}` in the Identity to access the dynamically
retrieved token.
